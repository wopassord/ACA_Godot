...
## Overview
This project is a Godot 4 (C#) port of the Elisa Virtual Agent. It features a procedural animation system (`FaceEngine`) that mathematically calculates and interpolates facial expressions and lip-syncing in real-time. The engine reads XML databases for action units and blendshapes, applying them dynamically to 3D meshes without relying on pre-baked animation clips. It also features a fully integrated BML (Behavior Markup Language) scheduler.
...
If you have never touched this project before, follow these steps to verify that the BML Parser and the mathematical engine are communicating correctly:
...
5. **Trigger the Action**: Once the window opens, **press the SPACEBAR**. You should see the agent smoothly perform a lip-sync sequence and transition into a "joy" facial expression. This proves that the raw BML string was successfully parsed, scheduled, and injected into the 3D mesh.
...
* **Phase 3: BML Synchronization (Completed)**
    * Migrate the `BmlSchedulerBehaviour` to act as the agent's central nervous system.
    * Parse Behavior Markup Language (BML) scripts via `Character.cs`.
    * Implement Text-To-Speech (TTS) audio pipeline using raw PCM data injection via Godot's `AudioStreamWav`.
    * *Current State: The face engine is fully autonomous, capable of parsing BML tags and executing scheduled expressions and visemes in real-time.*
